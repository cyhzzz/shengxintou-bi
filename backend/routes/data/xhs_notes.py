...
def get_xhs_notes_list_get():
    """
    小红书笔记列表（GET方法，适配OpenAPI规范）

    与 POST /xhs-notes-list 功能相同，但使用 GET 方法，
    参数通过 query string 传递，适配 React 新前端的 OpenAPI 规范。
    ...
    Returns:
        JSON: { success, notes, pagination, filters }
    """
    # 从 query string 获取参数
    ...
    # 广告策略筛选
    ad_strategy = request.args.get('ad_strategy')
    if ad_strategy:
        filters['ad_strategies'] = [ad_strategy]
    ...
    # 调用原有函数的处理逻辑
    return _process_xhs_notes_list(request_data)


def _process_xhs_notes_list(data):
    """处理小红书笔记列表查询（内部函数）"""
    ...
    try:
        ...
        for note in notes:
            ...
            notes_data.append({
                ...
                'note_title': note.note_title or '未知笔记',  # 兼容 OpenAPI 规范
                # 笔记类型（内容类型：图文笔记/视频笔记）
                ...
                'creator_name': note.producer or '未知',  # 兼容 OpenAPI 规范
                ...
                'impressions': int(note.total_impressions) if note.total_impressions else 0,  # 兼容 OpenAPI 规范
                'reads': int(note.total_clicks) if note.total_clicks else 0,
                'clicks': int(note.total_clicks) if note.total_clicks else 0,  # 兼容 OpenAPI 规范
                ...
                'cost': float(note.total_cost) if note.total_cost else 0,  # 兼容 OpenAPI 规范
                'is_ad': is_ad,
                'note_link': note.note_url,
                'note_url': note.note_url,  # 兼容 OpenAPI 规范
                # 互动指标
                ...
                'lead_users': int(note.lead_users) if note.lead_users else 0,
                'customer_mouth_users': int(note.customer_mouth_users) if note.customer_mouth_users else 0,
                'valid_lead_users': int(note.valid_lead_users) if note.valid_lead_users else 0,
                'opened_account_users': int(note.opened_account_users) if note.opened_account_users else 0,
                'valid_customer_users': int(note.valid_customer_users) if note.valid_customer_users else 0,
                'customer_assets_users': int(note.customer_assets_users) if note.customer_assets_users else 0,
                'customer_assets_amount': float(note.customer_assets_amount) if note.customer_assets_amount else 0,
                # 拆分数据（投放 vs 自然）
                ...
                'add_wechat_cost': add_wechat_cost,
...
def get_xhs_notes_list():
    """
    小红书笔记列表（POST方法，兼容旧前端）

    与 GET /xhs-notes/list 功能相同，但使用 POST 方法，
    参数通过 request body 传递，兼容旧版前端。
    ...
    """
    data = request.get_json()
    return _process_xhs_notes_list(data)
