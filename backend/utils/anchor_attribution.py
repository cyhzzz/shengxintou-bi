...
「客户来源 → (平台, 主播)」解析、纯人名 token 识别、复合来源均分只在此实现，
路由层一律 import，禁止再内联副本（此前 4 份副本曾导致改一处漏三处的口径漂移）。
消费方：/leads-detail/anchor-clusters、/anchor-clusters-trend、/anchor-weekly-analysis、
周报详细版直播板块（backend/routes/weekly_reports.py）。
移动端契约：scripts/check_api_contract.py ALGO_MARKERS 按本文件统计
复合来源均分行（div 赋值 max(n, 1)）出现次数，与 mobileHandlers 的 Math.max 写法对账。
"""
import re
...
# 复合来源分隔符："视频号引流-姚立琦,视频号引流-蒋亦凡" 拆成多个主播归因
SOURCE_SPLIT_PATTERN = re.compile(r"[,，;；、]+")
...
def load_live_type_mappings():
    """加载 dim_anchor_live_type 映射（仅 is_active），返回四元组。

    (token_to_anchor, token_to_live_type, anchor_to_live_types, plain_name_tokens)
    - token_to_anchor: source_token -> 归一化 anchor_name（错字校正）
    - token_to_live_type: source_token -> live_type
    - anchor_to_live_types: 归一化 anchor_name -> set(live_type)
    - plain_name_tokens: 纯人名 token（不含"引流-"/"直播带货-"，如"黄天平"，分支投顾自IP）
    """
    ...
    for r in lt_rows:
        if not r.is_active:
            continue
        token_to_anchor[r.source_token] = r.anchor_name
        token_to_live_type[r.source_token] = r.live_type
    ...
    return token_to_anchor, token_to_live_type, anchor_to_live_types, plain_name_tokens


def build_source_filter(plain_name_tokens):
    """客户来源基础过滤：非空 +（含"引流-" 或 命中纯人名 token）。

    纯人名 token（如"黄天平"）不含"引流-"，仅 like('%引流-%') 会漏掉这些记录。
    """
    source_filter = and_(
...
def compute_anchor_cluster_items(sd, ed, platforms_filter, agencies_filter, live_types_filter):
    ...
    token_to_anchor, token_to_live_type, anchor_to_live_types, plain_name_tokens = load_live_type_mappings()
    ...
    for r in rows:
        src = (r.客户来源 or "").strip()
        matches = []
        for part in SOURCE_SPLIT_PATTERN.split(src):
            segment = part.strip()
            if not segment:
                continue
            m = ANCHOR_SOURCE_PATTERN.match(segment)
            if m:
...
def build_anchor_clusters_payload(items, top_n):
    """主播聚类响应组装：totals + live_type_breakdown + meta。返回 {'data': ..., 'meta': ...}。"""
    totals = {
    ...
    return {
        ...
        'meta': {
            ...
            'pattern': ANCHOR_SOURCE_PATTERN.pattern,
...
def anchor_clusters_trend_response(granularity, filters):
    """主播引流走势计算体（原 /leads-detail/anchor-clusters-trend 逻辑，v3.1.27 / v3.3.0 加 live_types 过滤）。

    返回 data 字典（含 meta），路由壳直接 jsonify({'success': True, 'data': 返回值})。
    """
    ...
    # 预加载 dim_anchor_live_type 映射，构建 plain_name_tokens
    # 纯人名 token（如"黄天平"）不含"引流-"，原有的 like('%引流-%') 过滤
    # 会漏掉这些记录，导致日历年度总开户数偏少
    token_to_anchor, token_to_live_type, _, plain_name_tokens = load_live_type_mappings()

    source_filter = build_source_filter(plain_name_tokens)
    ...
    if live_types_filter:
        wanted = set(live_types_filter)
        wanted_tokens = {tok for tok, lt in token_to_live_type.items() if lt in wanted}
    ...
    for r in rows:
        src = (r.客户来源 or '').strip()
        tokens = [t.strip() for t in SOURCE_SPLIT_PATTERN.split(src) if t.strip()]
        ...
        matched = {t for t in tokens if ANCHOR_SOURCE_PATTERN.match(t) or t in plain_name_tokens}
        n = len(matched)
        div = max(n, 1)
        period = r.period
...
def anchor_weekly_analysis_response(filters, top_n):
    """主播周度拿量 + 各环节转化率分析计算体（原 /leads-detail/anchor-weekly-analysis 逻辑）。
    ...
    返回 {'data': ..., 'meta': ...}，路由壳直接 jsonify({'success': True, **返回值})。
    """
    sd = filters.get('start_date')
    ed = filters.get('end_date')
    platforms_filter = filters.get('platforms') or []
    ...
    # Python 端按 (anchor, week) 二次聚合（复用主播聚类的 token 拆分逻辑）
