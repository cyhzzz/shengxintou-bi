...
- `conversion_appmarket`（应用市场下载链路）：
  - 增量模式（前端开关关闭"全量替换"）：导入前先按 `设备号 + 下载日期` 删除匹配的旧记录，再 append 新记录，保留其他日期的数据。去重键为 `设备号 + 下载日期`（业务上可唯一识别同一条线索）。
  - 全量替换（开关开启）：**按日期分区替换**——保留 `2026-06-30`（含）之前的历史数据，只清空并重写 `2026-07-01` 及以后的数据（后续上传的文件仅含 7 月 1 日以后数据，禁止清空整表丢失历史）。边界常量见 `raw_import.APPMARKET_CONV_REPLACE_FROM`。
