...
- 指标列序必须跟随业务链路顺序：数量列按"曝光→点击→线索→APP激活→开户→有效户"排列，成本列紧贴对应数量列后（线索成本→APP激活成本→开户成本）。新增转化指标时先确认业务链路位置，不要默认追加到表格末尾。
- 新增业务字段时 `fetchData` 的 map 必须显式赋值所有类型字段；TypeScript 可选字段 `?:` 会让漏赋值通过 typecheck，需手动核对类型定义与 map 返回对象字段一致。
