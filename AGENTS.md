...
`website/` 静态页面（含三端下载入口）；`website/app.js` 调 GitHub API 直接触发最新 release 资产下载；`.github/workflows/pages.yml` 自动部署 website/ + frontend-react/dist/ 到 GitHub Pages（根路径官网，`/app/` 子路径 PWA）；push main 或 push tag `v*` 均触发部署（发版必改 version.json，故 tag 提交必重建 PWA）；tag 部署依赖仓库 Settings → Environments → github-pages 放行 `v*` 标签策略（属仓库设置、必须为 tag 类型，详见 `docs/rules/testing-and-delivery.md` 第 8 节）。
...
- 发版脚本会修改版本、commit、tag 和 push；只有用户明确要求发版时运行。发版流程：更新 version.json → commit → `git tag vX.Y.Z` → push main + push tag（缺一不可）——tag 触发 `release.yml` 打包 exe/APK/frontend-dist.zip 挂 release（先等 CI 全绿），tag 与 main push 均触发 `pages.yml` 重建部署 PWA（release 说明自动取自 version.json.changelog），无需本地构建。
