@echo off
REM ============================================================
REM 省心投 BI - 一键发布（本地打 tag + push，后续手动构建 + 上传）
REM
REM 用法：
...
REM   7. 开发者本地跑 scripts\build-installer.ps1 + scripts\build-apk.ps1
REM      构建产物后用 gh release upload 上传到 GitHub Release
REM
REM 历史：v3.5.x 曾依赖 GitHub Actions release.yml 自动构建，但 CI 长期
REM       不可用（push.tags 触发的 workflow run 始终 0 jobs 失败），
REM       v3.6.0 关闭该 workflow 并改为本地手动发布。
REM ============================================================
...
echo   4. 本地跑 scripts\build-installer.ps1（Windows） + cd android ^&^& npm run build:apk（Android）
echo   5. 用 gh release upload 把产物上传到 https://github.com/cyhzzz/shengxintou-bi/releases/tag/v%NEW_VER%
echo.
