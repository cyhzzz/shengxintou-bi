...
def get_agency_analysis():
    ...
    try:
        ...
        summary_query = db.session.query(
            ...
            func.sum(DailyMetricsUnified.customer_assets_amount).label('total_customer_assets_amount'),
            func.sum(DailyMetricsUnified.customer_assets_users).label('total_customer_assets_users'),
            func.sum(DailyMetricsUnified.existing_customers_assets).label('total_existing_customers_assets')
        )
        ...
        grand_total = {  # 全部合计
            ...
            'customer_assets_amount': 0,
            'customer_assets_users': 0,
            'existing_customers_assets': 0
        }
        ...
        for row in summary_results:
            ...
            customer_assets_amount = float(row.total_customer_assets_amount) if row.total_customer_assets_amount else 0
            customer_assets_users = int(row.total_customer_assets_users) if row.total_customer_assets_users else 0
            existing_customers_assets = float(row.total_existing_customers_assets) if row.total_existing_customers_assets else 0
            ...
            item = {
                ...
                'metrics': {
                    ...
                    'customer_assets_amount': customer_assets_amount,
                    'customer_assets_users': customer_assets_users,
            ...
            platform_subtotals[platform]['customer_assets_amount'] += customer_assets_amount
            platform_subtotals[platform]['customer_assets_users'] += customer_assets_users
            platform_subtotals[platform]['existing_customers_assets'] += existing_customers_assets
            ...
            grand_total['customer_assets_amount'] += customer_assets_amount
            grand_total['customer_assets_users'] += customer_assets_users
            grand_total['existing_customers_assets'] += existing_customers_assets
        ...
        for platform, metrics in platform_subtotals.items():
            ...
            platform_subtotal_rows.append({
                ...
                'metrics': {
                    ...
                    'customer_assets_amount': metrics['customer_assets_amount'],
                    'customer_assets_users': metrics['customer_assets_users'],
                    'existing_customers_assets': metrics['existing_customers_assets'],
                    'lead_cost': round(lead_cost, 2),
                    'account_cost': round(account_cost, 2)
                }
            })
        ...
        grand_total_row = {
            ...
            'metrics': {
                ...
                'customer_assets_amount': grand_total['customer_assets_amount'],
                'customer_assets_users': grand_total['customer_assets_users'],
                'existing_customers_assets': grand_total['existing_customers_assets'],
...
def get_dashboard_core_metrics():
    ...
    try:
        ...
        if agencies:
            new_customers_assets_query = new_customers_assets_query.filter(
                BackendConversions.agency.in_(agencies)
            )
            existing_customers_assets_query = existing_customers_assets_query.filter(
                BackendConversions.agency.in_(agencies)
            )
        ...
        if business_models:
            # 业务模式筛选需要通过DailyMetricsUnified表关联，因为BackendConversions表没有直接的business_model字段
            # 我们需要先查询符合条件的客户标识，再应用到资产查询
            from sqlalchemy import exists
            from sqlalchemy.orm import aliased

            # 先获取符合条件的DailyMetricsUnified记录
            dm_alias = aliased(DailyMetricsUnified)

            # 构建子查询获取符合条件的客户标识
            valid_customers_subquery = db.session.query(
                func.concat(
                    BackendConversions.platform_source, '|',
                    func.coalesce(BackendConversions.wechat_nickname, ''), '|',
                    func.coalesce(BackendConversions.capital_account, ''), '|',
                ...
                dm_alias,
                and_(
                    BackendConversions.lead_date == dm_alias.date,
                    BackendConversions.platform_source == dm_alias.platform,
                    BackendConversions.agency == dm_alias.agency
                )
            ).filter(
                and_(
                    BackendConversions.lead_date >= start_date,
                    BackendConversions.lead_date <= end_date,
                    dm_alias.business_model.in_(business_models)
                )
            ).subquery()
