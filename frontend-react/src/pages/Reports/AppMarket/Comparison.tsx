...
const AppMarketComparisonPage: React.FC = () => {
  ...
  const radarOption: EChartsOption = useMemo(() => {
    ...
    markets.forEach((m: any) => {
      ...
      indicators[3].max = Math.max(indicators[3].max, m.counts['入金'] || 0);
      indicators[4].max = Math.max(indicators[4].max, m.counts['有效户'] || 0);
    });
    return {
      tooltip: {},
    ...
    return {
      ...
      series: markets.map((mk) => ({
        ...
        data: months.map((m) => {
          ...
          return row ? row.counts['开户成功'] || 0 : 0;
        }),
    ...
    <div className={styles.page}>
      ...
      <Spin spinning={loading}>
        <Row gutter={16} style={{ marginTop: 16 }}>
          ...
          <Col span={12}>
            <Card title='月度堆叠柱状图（按开户成功）' size='small'>
