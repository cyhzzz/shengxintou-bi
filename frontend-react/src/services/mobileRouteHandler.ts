...
async function handleKosWeekly(body: any): Promise<any> {
  const sd = body?.start_date;
  const ed = body?.end_date;
...
async function handleKosWeeklyFilterOptions(): Promise<any> {
  const leads = await kosGetLeads();
  return {
    platform: KOS_PLATFORM,
    roster: KOS_ROSTER,
...
async function handleDataFreshness(): Promise<Record<string, any>> {
...
export async function mobileRouteHandler(url: string, body: any): Promise<any> {
  ...
  switch (path) {
    ...
    // v3.8.0：小红书分支KOS转化周报（移植自 xhs_kos_weekly.py）
    case 'xhs/kos-weekly':
