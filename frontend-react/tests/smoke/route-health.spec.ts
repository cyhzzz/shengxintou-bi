...
const PUBLIC_ROUTES: { name: string; path: string }[] = [
  ...
  // v4.1.9：智能诊断引擎（菜单名「数据体检」，桌面/Web/移动端均注册）
  { name: '数据体检', path: '/diagnosis' },
  // v4.2.0：AI 分析报告（LLM 跨月趋势对比；showIntelligentAnalysis=true 时注册，桌面/Web 端覆盖）
  { name: 'AI 分析报告', path: '/intelligent-analysis' },
  ...
  // v3.9.1：广告计划分析（结合 dim_ad_plan_class + fact_conv_appmarket + agg_vendor_daily）
