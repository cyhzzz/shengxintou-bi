...
test.describe('简称映射管理页面功能测试', () => {
  test.beforeEach(async ({ page }) => {
    await navigateToPage(page, 'abbreviation-management');
  });
  ...
  test('数据表格 - 全称列显示', async ({ page }) => {
    await waitForDataLoad(page);
    
    const fullNameColumn = page.locator('th:has-text("全称"), th:has-text("完整名称")').first();
    const hasFullNameColumn = await fullNameColumn.isVisible({ timeout: 3000 }).catch(() => false);
    console.log('全称列可见:', hasFullNameColumn);
  });

  test('数据表格 - 简称列显示', async ({ page }) => {
    await waitForDataLoad(page);
    
    const abbrColumn = page.locator('th:has-text("简称")').first();
    const hasAbbrColumn = await abbrColumn.isVisible({ timeout: 3000 }).catch(() => false);
    console.log('简称列可见:', hasAbbrColumn);
  });
  ...
  test('加载状态 - 页面加载状态', async ({ page }) => {
    ...
    const hasData = await hasDataInTable(page);
    console.log('简称映射管理页面有数据:', hasData);
  });
