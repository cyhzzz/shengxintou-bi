...
class AgencyAnalysisReport {
     ...
     * @param {boolean} options.hybridMode - 混合迁移模式（跳过筛选器渲染，使用外部容器）
     */
    constructor(options = {}) {
        this.filterBar = null;
        ...
        console.log(`[AgencyAnalysisReport] 初始化，混合模式: ${this.hybridMode}`);
    ...
    async init() {
        ...
        // 1. 渲染独立筛选器（混合模式下跳过，使用外部筛选器）
        if (!this.hybridMode) {
            this.renderFilters();
        } else {
            console.log('[init] 混合模式：跳过筛选器渲染，使用外部容器');
        }
    ...
    render() {
        // 混合迁移模式：使用外部容器
        if (this.hybridMode) {
            console.log('[render] 混合模式：使用外部容器渲染');
            return this.renderHybridMode();
        }
    ...
    /**
     * 混合迁移模式渲染
     * 使用外部 React/Ant Design 提供的容器
     */
    renderHybridMode() {
        console.log('[renderHybridMode] 检查外部容器...');

        // 检查图表容器
        const chartContainer = document.getElementById('trendChart');
        if (chartContainer) {
            console.log('[renderHybridMode] 找到外部图表容器 #trendChart');
        } else {
            console.warn('[renderHybridMode] 未找到图表容器 #trendChart');
        }

        // 检查/创建表格容器
        let tableContainer = document.getElementById('agencyTable');
        if (tableContainer) {
            console.log('[renderHybridMode] 找到外部表格容器 #agencyTable');

            // 检查是否已有表格结构
            if (!tableContainer.querySelector('table.data-table')) {
                console.log('[renderHybridMode] 创建表格结构...');
            ...
            console.warn('[renderHybridMode] 未找到表格容器 #agencyTable');
        }

        // 延迟渲染图表和表格
        setTimeout(async () => {
            console.log('[renderHybridMode] 渲染图表...');
            await this.renderChart();
            console.log('[renderHybridMode] 渲染表格...');
            this.renderTable();
        }, 100);
    }

    /**
     * 设置当前图表指标（供外部/React组件调用）
     * @param {string} metric - 指标名称
     */
    setCurrentMetric(metric) {
        if (!this.metrics[metric]) {
            console.warn(`[setCurrentMetric] 无效的指标: ${metric}`);
            return;
        }

        console.log(`[setCurrentMetric] 设置指标: ${metric}`);
    ...
    renderTable() {
        ...
        // 混合模式：确保表格结构存在
        let tbody = document.querySelector('#agencyTable tbody');
        if (!tbody) {
            // 混合模式下，表格容器可能是 div，需要创建表格结构
            const container = document.getElementById('agencyTable');
            if (container && this.hybridMode) {
                console.log('[renderTable] 混合模式：创建表格结构...');
                container.innerHTML = `
                    <table class="data-table">
                        <thead>
                            <tr>
            ...
            if (!tbody) {
                console.warn('[renderTable] 找不到表格 tbody');
                return;
            }
        }
